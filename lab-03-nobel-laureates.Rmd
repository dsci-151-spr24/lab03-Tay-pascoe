...
***There are `r nrow(935)` rows and `r ncol(26)` columns. Each row represents a nobel winner.***
...
ggplot(data = nobel_living_science, aes(y = country_us)) +
  ...
  facet_wrap(~category)
```
***It appears that there most Nobel Lauretes were based in the US when they won their prizes. I think this data does support the headline because even though they were in the US when they won their prizes, many were immigrants and thus proves that immigration is important for American Science. ***
...
nobel_living_science %>% group_by(born_country_us)%>% count()
```
***There are 105 US born winners***
...
ggplot(data = nobel_living_science, aes(y = country_us, fill=born_country_us)) +
  ...
  facet_wrap(~category)
```
***I think this data show that the buzzfeed article is correct. Even in our data previously with winners and where they were born, there was only an 18 person difference between the other and US which is quite close. This data shows that they are pretty even but even still, more were born in other countries***
...
```
***the most common country was Germany which had 20 and then japan with 17 and UK with 16***
